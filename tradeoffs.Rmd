...
["Broad" integration tests](https://martinfowler.com/bliki/IntegrationTest.html) are run against live services, and thus require no test doubles.
...
```{r minimal-wrapper}
# unexported helper
# this is not tested, b/c the implementation could change
verb_doi_handle <- function(doi, ...) {
  httr::VERB(
    url = "https://doi.org/",
    # better to use curl::curl_escape() here
...
test_that("Existing DOIs are found", {
  ...
  # this snapshot will also capture some change in the API output
  expect_snapshot_output(get_doi_handle("10.1038/nature14236"))
