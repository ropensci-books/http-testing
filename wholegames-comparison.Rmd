...
For creating API errors, and API sequence of responses (e.g. 502 then 200), you end up either using webmockr, or amending mock files.[^seq-httptest]
...
With presser, debugging works this way: load the helper or test file where 
...
With vcr, you'll have to load the helper file or source the setup file after making sure the paths use in it work both from `tests/testthat/` and the package root (see `?vcr::vcr_test_path`), and then use `vcr::inject_cassette()`; don't forget to run `vcr::eject_cassette()` afterwards.
