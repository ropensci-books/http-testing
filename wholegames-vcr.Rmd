...
* loads vcr, 
* indicates where mocked responses are saved (`"../fixtures"` which translates, from the root of the package, to `tests/testthat/fixtures`), 
* and checks that you are not using the same name twice for cassettes (mock files).

We had to tweak vcr setup a bit for our needs.
...
The key function will be `vcr::use_cassette("cassette-informative-and-unique-name", {<code-block>})` which tells vcr to create a mock file to store all API responses for API calls occurring in the code block.
...
* We set up vcr usage in our package exemplighratia by running `use_vcr()` and tweaking the setup file to fool our own package that needs an API token.
