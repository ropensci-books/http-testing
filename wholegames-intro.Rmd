...
Our minimal package, [`exemplighratia`](https://github.com/maelle/exemplighratia), accesses the GitHub status API and one endpoint of GitHub V3 REST API.
...
Our example package calls web _APIs_ but the tools and concepts are applicable to packages wrapping any web resource, even poorly documented ones.[^undocumented]
:::
...
In the following chapters, we'll add more robust testing infrastructure to this minimal package, and we will do that _three_ times to compare packages/approaches: once with [vcr](#vcr), once with [httptest](#httptest), and once with [webfakes](#webfakes).
