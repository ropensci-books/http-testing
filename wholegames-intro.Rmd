...
Similar to the [Whole Game chapter in the R packages book by Hadley Wickham and Jenny Bryan](https://r-pkgs.org/whole-game.html), we will go through how to add HTTP tests to a minimal package.
However, we will do it _four_ times to present alternative approaches: with [vcr](#vcr), [httptest](#httptest), [httptest2](#httptest2), [webfakes](#webfakes).
After that exercise, we will compare approaches: we will compare both packages that involve mocking i.e. [vcr vs. httptest](#mocking-pkgs-comparison); and all three HTTP packages in a [last chapter](#pkgs-comparison).
...
For the sake of having an example of a package _requiring_ authentication we will assume the API is *not* usable without authentication.
...
* `use_r("organizations.R")` (and `usethis::use_package("cli")`) to add a second function. 
Note that an ideal version of this function would have some sort of callback in the retry, to call the `gh_api_status()` function (maybe with `httr2::req_retry()`'s `is_transient` argument).
