...
* To add a second function, `use_r("organizations.R")`. Note that an ideal version of this function would have some sort of callback in the retry, to call the `gh_api_status()` function (which seems easier to implement with [crul's retry method](https://blog.r-hub.io/2020/04/07/retry-wheel/#retry-in-crul)).
...
* To add a simple test, `use_test("organizations")`.
