...
For this, we start from the initial state of exemplighratia again. 
Back to square one!
...
[Corresponding pull request to exemplighratia](https://github.com/ropensci-books/exemplighratia/pull/9/files).
Feel free to fork the repository to experiment yourself!
:::
...
We will tweak it a bit to fool our package into believing there is an API token around in contexts where there is not.
Since tests will use recorded responses when we are not recording, we do not need an actual API token when not recording, but we need `gh_organizations()` to not stop because `Sys.getenv("GITHUB_PAT")` returns nothing.
