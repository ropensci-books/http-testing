...
[Corresponding pull request to exemplighratia](https://github.com/maelle/exemplighratia/pull/3/files)
:::
...
This one makes sure to load httptest.
...
We will use two ways to add tests

* With a function that automatically records API responses in a mock file if said mock file doesn't already exist (which will remind you of `vcr::use_cassette()`);
* By creating mock files manually.
...
* We first run the test as is. Note the `simplify = FALSE` that means the mock file also contains headers for the response.
...
For the full list of changes applied to exemplighratia in this chapter, see [the pull request diff on GitHub](https://github.com/maelle/exemplighratia/pull/3/files).
