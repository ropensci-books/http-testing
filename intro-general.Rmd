...
In your package, is information is getting back and forth between the R session and internet, you are using some sort of HTTP tooling.
...
A response has a status code indicating whether the request succeeded, headers and it can have a body.

Hopefully the documentation of the web API you are working with shows good examples of responses.
...
* httr is the most popular and oldest of the two, and supports OAuth among other things. httr docs feature a vignette called [Best practices for API packages](https://httr.r-lib.org/articles/api-packages.html)

* crul does not support OAuth but it uses an object-oriented interface, which you might like.
crul has a set of [clients, or ways to perform requests](https://docs.ropensci.org/crul/articles/choosing-a-client.html), that might be handy.
